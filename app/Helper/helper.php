...
function instructor_available_balance()
{
    ...
    $total_withdraw_balance = Withdraw::where('user_id', Auth::id())->whereIn('status', [0, 1])->sum('amount');
...
{
    if (\App\Models\Take_exam::whereUserId(Auth::id())->whereExamId($exam_id)->count() > 0) {
        return 'yes';
    } else {
        return 'no';
    }
...
{
    if (\App\Models\Answer::whereUserId(Auth::id())->whereExamId($exam_id)->whereQuestionId($question_id)->whereQuestionOptionId($question_option_id)->count() > 0) {
        $answer = \App\Models\Answer::whereUserId(Auth::id())->whereExamId($exam_id)->whereQuestionId($question_id)->whereQuestionOptionId($question_option_id)->orderBy('id', 'DESC')->first();
...
{
    $exam = \App\Models\Exam::find($exam_id);
    $number_of_correct_answer = \App\Models\Answer::whereUserId(Auth::id())->whereExamId($exam_id)->whereIsCorrect('yes')->count();
...
function get_position($exam_id)
{
    ...
    if (array_key_exists(Auth::id(), $list)) {
        return $list[Auth::id()];
    } else {
        return '0';
    }
...
{
    $instructor_notifications = \App\Models\Notification::where('user_id', Auth::id())->where('user_type', 2)->where('is_seen', 'no')->orderBy('created_at', 'DESC')->get();
    $student_notifications = \App\Models\Notification::where('user_id', Auth::id())->where('user_type', 3)->where('is_seen', 'no')->orderBy('created_at', 'DESC')->get();
...
function studentCourseProgress($course_id, $enrollment_id)
{
    ...
    if ($course->course_type == COURSE_TYPE_GENERAL) {
        ...
        $number_of_total_view_lecture = \App\Models\Course_lecture_views::where('course_id', $course_id)->where('enrollment_id', $enrollment_id)->where('user_id', Auth::id())->count();
        $result = 0;
