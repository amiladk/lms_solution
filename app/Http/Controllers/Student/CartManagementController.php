    ...
    public function addToCart(Request $request)
    {
        ...
        try {
            if ($request->course_id) {
                $enrollment = Enrollment::where(['course_id' => $request->course_id , 'user_id' => Auth::id(), 'status' => ACCESS_PERIOD_ACTIVE])->whereDate('end_date', '>=', now())->first();
            ...
            if ($request->bundle_id) {
                ...
                $ownBundleCheck = Bundle::whereUserId(Auth::id())->where('id', $request->bundle_id)->first();
                if ($ownBundleCheck) {
                ...
                $enrollment = Enrollment::where(['user_id' => Auth::id(), 'bundle_id' => $request->bundle_id, 'status' => ACCESS_PERIOD_ACTIVE])->whereDate('end_date', '>=', now())->first();
                if ($enrollment) {
                    $order = Order::find($enrollment->order_id);
                    if ($order) {
                        if ($order->payment_status == 'due') {
            ...
            if ($request->course_id) {
                $cartExists = CartManagement::whereUserId(Auth::id())->whereCourseId($request->course_id)->first();
            } elseif ($request->product_id) {
                $cartExists = CartManagement::whereUserId(Auth::id())->whereProductId($request->product_id)->first();
            } elseif ($request->bundle_id) {
                $cartExists = CartManagement::whereUserId(Auth::id())->whereBundleId($request->bundle_id)->first();
            }
            ...
            $response['quantity'] = CartManagement::whereUserId(Auth::id())->count();
    ...
    public function addToCartConsultation(Request $request)
    {
        ...
        if ($request->consultation_slot_id) {
            ...
            try {
                ...
                $consultationOrderExit = Order_item::whereUserId(Auth::id())->whereConsultationSlotId($request->consultation_slot_id)->where('consultation_date', $request->bookingDate)->first();
                ...
                $cartExists = CartManagement::whereUserId(Auth::id())->whereConsultationSlotId($request->consultation_slot_id)->where('consultation_date', $request->bookingDate)->first();
                if ($cartExists) {
                    $response['msg'] = __("Already added to cart!");
                    $response['status'] = 409;
                    DB::rollBack();
